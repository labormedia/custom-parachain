...
pub type ChainSpec =
    sc_service::GenericChainSpec<custom_parachain::RuntimeGenesisConfig, Extensions>;

/// The default XCM version to set in genesis config.
const SAFE_XCM_VERSION: u32 = xcm::prelude::XCM_VERSION;
const JIN_PARA_ID: u32 = 4511;
use hex::FromHex;
...
pub fn get_authority_keys_from_seed(s: &str) -> (AccountId, AuraId) {
	(
...
/// have just one key).
pub fn template_session_keys(keys: AuraId) -> custom_parachain::SessionKeys {
    custom_parachain::SessionKeys { aura: keys }
}

pub fn development_config() -> ChainSpec {
    // Give your base currency a unit name and decimal places
    let mut properties = sc_chain_spec::Properties::new();
    properties.insert("tokenSymbol".into(), "JIN".into());
